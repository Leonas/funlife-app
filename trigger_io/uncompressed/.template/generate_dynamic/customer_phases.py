...
def include_platform_in_html(server=False):
	return [
		...
		{'when': {'platform_is': 'web'}, 'do': {'find_and_replace_in_dir': {
			...
			"find": "<head>",
			"replace": "<head><script src='%{back_to_parent}%_forge/all.js'></script>"
		}}},
...
def include_requirements():
	return [
		{'when': {'platform_is': 'ios', 'config_property_exists': 'modules.requirements.ios.minimum_version'}, 'do': {'set_in_biplist': {
			"filename": 'development/ios/*/Info.plist',
			"key": "MinimumOSVersion",
			"value": "${modules.requirements.ios.minimum_version}"
		}}},
	]
